...
//		CopyObject.write(parts, "parts.txt");
	...
	public void run() {
		...
		if (Global.modifyDiffMatrix){
			SaveworthyPuzzleData pd = SaveworthyPuzzleData.loadObject(Global.pathToPuzzleDataInit + Global.INIT_MOD_DATA_FILE);
			diffMatrix = pd.diff;
		}
		...
		try {
			...
			if (Global.modifyDiffMatrix || Global.modifyConf){
		...
		for (int i = 0; i < partsNum; i++) {
			for (int j = 0; j < partsNum; j++) {
				if (chosenParts.containsKey(i) && chosenParts.containsKey(j)) {
					...
					diffMatrix[1][j][i] = Float.MAX_VALUE;
					diffMatrix[0][j][i] = Float.MAX_VALUE;
					diffMatrix[3][j][i] = Float.MAX_VALUE;
					diffMatrix[2][j][i] = Float.MAX_VALUE;
					...
					if (finalPlacement[nextA[1] - 1 + partsNum / 4][nextA[0]
						...
						diffMatrix[0][j][i] = Float.MAX_VALUE; //Dov added the other direction
					...
					if (finalPlacement[nextA[1] + 1 + partsNum / 4][nextA[0]
						...
						diffMatrix[1][j][i] = Float.MAX_VALUE; //Dov added the other direction
					...
					if (finalPlacement[nextA[1] + partsNum / 4][nextA[0]
						...
						diffMatrix[2][j][i] = Float.MAX_VALUE; //Dov added the other direction
					}
					if (finalPlacement[nextA[1] + partsNum / 4][nextA[0]
							+ partsNum / 4 + 1] != null){
						diffMatrix[2][i][j] = Float.MAX_VALUE;
						
						diffMatrix[3][j][i] = Float.MAX_VALUE; //Dov added the other direction
