...
public class Global {
	...
	public final static String PUZZLE_NAME_SUFFIX = "a";
	public static final int ORIGINAL_PART_SIZE = 28;
	public static boolean LOAD_BACKUP_SCRIPT = false;
	public static final String SCRIPT_NAME = "backup_script1";

	public final static int[][]extrapolationVals = {{5,1,3,0,100}}; //0-patchSize(0 means don't extrapolate), 1-numIterations, 2-burnExtent, 3-sizeOverlap (0 means use old calcDiff version), 4-patchratio
	public final static int[]diffBlockSizeVals = {1,2}; //Instead of comparing pixel to pixel, the average of pixel blocks of this size will be compared
//	public final static double[]colorDiffRatioVals = {1}; //N.I.U weights for using color vs gradients in CalcDiff. relevant only for the case with overlap.
	public final static int[]diffNormVals = {2};	//Which norm to use when comparing pixels/pixel block averages. Original value was 2.
	public final static double[]lFactorVals = {1}; // L weight when comparing pixels in LAB. (A and B have weight 1).
//	public final static double[] saliencyFactorVals = {0}; //N.I.U Weights for the saliency channel to be combined with the LAB comparison.
	public final static int[] modifyConfVals = {1};		//Use Matlab Hole Filling to modify confidence, 0-don't use, 1-use
	public final static int[] useImportanceVals = {0};	//0-don't use, 1-use
	public final static double[] geometricRelationsVals = {0}; //The threshold for vetoing a placing of puzzle part. If the expected geometric location is further than this value - don't place.
	public final static double[] geometryKeyConstantVals = {0}; //Constants to control how the geometry key (for the pool) will be calculated from the inverse of the distance from expected location
	...
	public static enum ParameterNames {extrapolationVals,diffBlockSizeVals,diffNormVals,lFactorVals,
										modifyConfVals, useImportanceVals, geometricRelationsVals, geometryKeyConstantVals,size;}
	public static enum FirstPieceChoice {normal,geometry,saliency;}
	public static BackupScript backupScript;
	//Final String values
	public static final String PROJECT_PATH = "C://Users//dov//workspace//Puzzle Resources//";
	public static final String geometricRelationsFile = "_gm.txt";
	public static final String SCORES = PROJECT_PATH + "Scores//"; // Where the text puzzle score files will be stored
	public static final String PUZZLE_DATA = PROJECT_PATH + "Data//"; //Where the data needed for the GuiDebugger and puzzleGui will be stored
	public static final String scoresFileName = "all_scores_" +PUZZLE_NAME_SUFFIX+".txt";
	public static final String INIT_DATA_FILE = "_init_data.txt"; //The file that holds the diff, conf rank data etc.. at the begining.
	public static final String INIT_MOD_DATA_FILE = "_init_mod_data.txt";//The file that holds the diff, conf rank data etc.. at the begining but after modified by hole filling in matlab
	public static final String PUZZLE_STATES_FILE = "_states_";	//The suffix for the name of the puzzle data file that will be used by the GuiDebugger	
	public static final String FILE_TYPE = ".png";
	public static final String IMPORTANCE_TYPE = "_importance_"; //The folder name for the importance maps. Allows each puzzle to have different importance options to choose from
	...
	public static final String INDIVIDUAL_PARTS_FOLDER = PROJECT_PATH+ "Individual Parts//"; //Extrapolated parts will be stored here
	...
	public static boolean no_matlab = false; //Will be set to true when running java code from matlab and deactivate a possibility of running matlab from java.
	...
	public static final boolean savePartImages = true; //Save the parts from the extrapolation (and not just the combined image)
	...
	public static final boolean createExtrapolationImportance = true; //Create importance maps during the extrapolation process (based on the quality of patch matching
	...
	public static final boolean positiveConf = true; //Use the new Confidence 
	
//	public static final boolean calcErrorBaseLine = true;	//

	public static final boolean modifyDiffMatrix = false;	//Swap the diff matrix before starting to solve the puzzle with a pre-made matrix from the file INIT_MOD_DATA_FILE
	...
	public static final boolean MAX_CONF_IN_POOL = true;	//When adding a candidate to the pool, should its score be affected by the maximum conf he has with one of the neighbors?(rather than average?)
	...
	public static final float MOD_CONF_BASELINE = 5;	//If the "modifyConf" is used, Parts with conf under this value will attempt a modification (with Matlab Hole Filling)
...
//			parameters[ParameterNames.saliencyFactorVals.ordinal()] = new DoubleParameter(ParameterNames.saliencyFactorVals,saliencyFactorVals);
			...
			parameters[ParameterNames.geometricRelationsVals.ordinal()] = new DoubleParameter(ParameterNames.geometricRelationsVals,geometricRelationsVals);
			parameters[ParameterNames.geometryKeyConstantVals.ordinal()] = new DoubleParameter(ParameterNames.geometryKeyConstantVals,geometryKeyConstantVals);
	...
	public static String getPreparedName(String puzzleName, int patchSize, int numIterations, int sizeOverlap, int burnExtent, int patchRatio){
		return puzzleName+"_Prepared_PS"+patchSize+"_NIt"+numIterations+"_SO"+sizeOverlap+"_BE"+burnExtent+"_PR"+patchRatio;
	}
	...
	public static String getPuzzleID(){
		String solveParameters = "_DBS"+diffBlockSize  + "_DN" + diffNorm + "_LF" + lFactor + "_UI" + boolToInt(useImportance) + "_MC" + boolToInt(modifyConf) +
								 "_GRT " + geometricRelationsThreshold + "_GRKC" + geometryKeyConstant;
